...
let phoneNumbersIfBalanceGreater2000 = [];

for (let i = 0; i < users.length; i++) {
    let user = users[i];
    if (user.balance > 2000) {
        phoneNumbersIfBalanceGreater2000.push(user.phone);
    }
    
}

console.log(phoneNumbersIfBalanceGreater2000);

let sumOfBalances = 0;

for (let i = 0; i < users.length; i++) {
    let user = users[i];
    sumOfBalances += user.balance;
}
